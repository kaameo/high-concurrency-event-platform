...
## HikariCP 풀 사이즈와 Sync 병목의 관계

### 왜 Sync가 느린가?

DB INSERT 자체는 수 ms이지만, **HikariCP 커넥션 풀(기본 10개)을 500 VU가 경합**하는 대기 시간이 latency의 대부분을 차지한다.

```
Kafka produce (500 VU 동시):
  VU 1~500 → 동시 produce → 각 2ms → 완료 (커넥션 풀 제한 없음)

DB INSERT (500 VU 동시):
  VU 1~10   → 커넥션 획득 → INSERT 5ms → 반환
  VU 11~20  → 대기 50ms + INSERT 5ms
  VU 491~500 → 대기 200ms+ ...
```

### 실무 HikariCP 풀 사이즈 기준

PostgreSQL 공식 권장 공식: `connections = (CPU 코어 수 × 2) + 유효 디스크 수`

| 규모 | 풀 사이즈 | 비고 |
|------|----------|------|
| 소규모 (단일 인스턴스) | 10~20 | HikariCP 기본값 10 |
| 중규모 (2~5 인스턴스) | 10~30 | 인스턴스 × 풀 = 총 커넥션 |
| 대규모 (10+ 인스턴스) | 5~15 | DB 총 커넥션 한도(기본 100) 분배 |

풀을 무작정 늘리면 DB에서 컨텍스트 스위칭과 lock 경합이 증가해 오히려 성능이 저하된다. HikariCP 개발자도 "작은 풀이 더 빠르다"고 강조한다.

**결론: 고동시성에서는 풀 사이즈 증가 대신 Kafka 같은 버퍼링으로 DB 부하를 평탄화하는 것이 정석.**
